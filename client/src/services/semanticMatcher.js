/**
 * Client-Side Semantic Meaning Normalization & Vector Lesson Matcher
 * 
 * Maps recognized ISL glosses to canonical concept spaces and executes
 * cosine vector matching strictly against the active lesson's chunks.
 */
...
/**
 * Searches the currently selected lesson strictly and returns the top semantic chunk match.
 */
export function matchSignToLesson(gloss, currentLesson, similarityThreshold = 0.40) {
  ...
  // Extract chunks strictly from current lesson
  const rawChunks = [];
  ...
  if (isMatched && bestChunk) {
    return {
      ...
      explanation: `The recognized sign "${normalized.gloss}" directly matches the ${normalized.primaryConcept} explained in ${bestChunk.section}.`,
